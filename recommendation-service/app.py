...
movies = pd.read_csv('/mnt/data/movies.dat', sep="::", engine='python', names=["MovieID", "Title", "Genres"])
ratings = pd.read_csv('/mnt/data/ratings.dat', sep="::", engine='python', names=["UserID", "MovieID", "Rating", "Timestamp"])
users = pd.read_csv('/mnt/data/users.dat', sep="::", engine='python', names=["UserID", "Gender", "Age", "Occupation", "Zip-code"])

# Función para calcular recomendaciones basadas en género
def recommend_by_genre(movie_voted, user_id):
    # Obtener el género de la película votada
    ...
    best_movies = ratings[ratings['MovieID'].isin(similar_movies['MovieID'])].groupby('MovieID')['Rating'].mean().sort_values(ascending=False)
